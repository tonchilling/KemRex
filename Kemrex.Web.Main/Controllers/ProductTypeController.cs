...
{
    public class ProductTypeController : KemrexController
    {
        public ActionResult Index(int? page, int? size, string msg, AlertMsgType? msgType,
            string src = "")
        {
            List<TblProductType> lst = new List<TblProductType>();
            try
            {
                ...
                int total = uow.Modules.ProductType.Count(src);
                ...
                lst = uow.Modules.ProductType.Gets(Pagination.Page, Pagination.Size, src);
            }
            catch (Exception ex)
            {
        ...
        {
            TblProductType ob = uow.Modules.ProductType.Get(id ?? 0);
        ...
        public ActionResult SetDetail()
        {
            ...
            TblProductType ob = uow.Modules.ProductType.Get(id);
            if (ob.TypeId <= 0)
            {
                ob.CreatedBy = CurrentUID;
                ob.CreatedDate = CurrentDate;
            }
            ob.TypeName = Request.Form["TypeName"];
            ob.TypeOrder = Request.Form["TypeOrder"].ParseInt();
            ...
            try
            {
                ...
                uow.Modules.ProductType.Set(ob);
                uow.SaveChanges();
        ...
        public ActionResult Delete()
        {
            try
            {
                ...
                uow.Modules.ProductType.Delete(ob);
                uow.SaveChanges();
        ...
        private ActionResult ViewDetail(TblProductType ob, string msg, AlertMsgType? msgType)
        {
            try
            {
                if (ob == null)
