...
{
    public class PileController : BaseController
    {
        ...
        {
            TblPile ob = db.TblPile.Find(id ?? 0) ?? new TblPile();
        ...
        public ActionResult SetDetail()
        {
            ...
            TblPile ob = db.TblPile.Find(id) ?? new TblPile();
            ob.PileName = Request.Form["PileName"];
            ob.SeriesId = Request.Form["SeriesId"].ParseInt();
            //ob.Series = db.TblPileSeries.Find(ob.SeriesId.Value);
            ...
            try
            {
                ...
                if (ob.PileId <= 0) { db.TblPile.Add(ob); }
        ...
        private ActionResult ViewDetail(TblPile ob, string msg, AlertMsgType? msgType)
        {
            try
            {
                ...
                ViewData["optSeries"] = db.TblPileSeries.OrderBy(x => x.SeriesOrder).ToList();
                return View(ob);
            }
            catch (Exception ex)
            {
