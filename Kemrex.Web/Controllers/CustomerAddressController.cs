...
{
    public class CustomerAddressController :  KemrexController
    {
        public ActionResult Detail(int cusId, int? id, int? qtid, string msg, AlertMsgType? msgType)
        {
            TblCustomerAddress ob = uow.Modules.CustomerAddress.Get(id ?? 0);
            if (ob.AddressId <= 0)
            {
                ob.CustomerId = cusId;
        ...
        public ActionResult SetDetail()
        {
            ...
            ob.AddressName = Request.Form["AddressName"];
            ob.AddressValue = Request.Form["AddressValue"];
            int subdistrictId = Request.Form["SubDistrictId"].ParseInt();
            if (subdistrictId <= 0)
            {
                ob.SubDistrictId = null;
                ob.SubDistrict = null;
            }
            else
            {
                ob.SubDistrictId = subdistrictId;
                ob.SubDistrict = null; // uow.Modules.SubDistrict.Get(subdistrictId);
            }
            ob.AddressPostcode = Request.Form["AddressPostcode"];
            ob.AddressContact = Request.Form["AddressContact"];
            ob.AddressContactEmail = Request.Form["AddressContactEmail"];
            ob.AddressContactPhone = Request.Form["AddressContactPhone"];
            ob.AddressOrder = Request.Form["AddressOrder"].ParseInt();
            ...
            try
            {
                ...
                uow.Modules.CustomerAddress.Set(ob);
                uow.SaveChanges();
                if ((qtid ?? 0) > 0)
                {
        ...
        private ActionResult ViewDetail(TblCustomerAddress ob, int? qtid, string msg, AlertMsgType? msgType)
        {
            try
            {
                if (ob == null)
