    ...
    public class EmployeeUserPermissionModule : IModule<TblEmployeeUserPermission, int>
    {
            ...
            , string src = "")
        {
            var data = db.TblEmployeeUserPermission
                .AsQueryable();
            data = Filter(data, src).OrderBy(x => x.AccountId);
            if (size > 0)
        ...
        {
            if (!string.IsNullOrWhiteSpace(src)) { data = data.Where(x => x.AccountId.ToString().Contains(src)); }
            return data;
        }

        public bool IsExist(int id)
        { return db.TblEmployeeUserPermission.Where(x => x.AccountId == id).Count() > 0 ? true : false; }
        ...
        public void Set(List<TblEmployeeUserPermission> ob)
        {
            ...
            {
                db.TblEmployeeUserPermission.RemoveRange(db.TblEmployeeUserPermission.Where(o => o.FunId == ob[0].FunId && o.AccountId == ob[0].AccountId));
